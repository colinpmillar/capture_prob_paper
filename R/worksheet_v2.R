...
# ------------------------------------------------
# 
#  Full analysis version 2 : with pass information
# 
# ------------------------------------------------
...
ef $ pass23 <- factor(replace(pass, pass > 2, 2))
...
# 4413 samples in total
sum(tapply(ef$n, ef$sampleID, sum) > 0)
# of which 2918 had some fish
...
# 1453 unique sites with non-zero samples
...
# 487 samples observe only one lifestage
...
if (FALSE) {
  ...
  cat("Done!/n")
  save(llscreen, file = "llscreen.rData")
}
load("llscreen.rData")
...
mean(pchisq(dscreen, 1) > 0.95)
mean(pchisq(dscreen, 1) > 0.99)

# there look to be some outliers...
mean(pchisq(dscreen, 1)[dscreen > 1e-5] > 0.95)
mean(pchisq(dscreen, 1)[dscreen > 1e-5] > 0.99)
...
table(p.adjust(1-pchi, "BH") < 0.05)
table(p.adjust(1-pchi, "BH") < 0.01)

table(p.adjust(1-pchi, "holm") < 0.05)
table(p.adjust(1-pchi, "holm") < 0.01)
